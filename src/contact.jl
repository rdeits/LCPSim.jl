...
function add_contact_constraints(model::Model, point, obstacle, β, λ, c_n, D_transpose_times_v, separation_from_obstacle, normal_velocity, Δr, Δr_true)
    ...
    @disjunction(model,
                 (separation_from_obstacle <= 0) & (Δr.v .== Δr_true.v),
                 (c_n <= 0) & (Δr.v .== 0))


    # @disjunction(model,
    #              ((c_n == 0) & (normal_velocity >= 0)),
    #              ((normal_velocity == 0) & (separation_from_obstacle <= 0)),
    #              ((c_n == 0) & (separation_from_obstacle >= 0))
    #              )
...
function resolve_contact(xnext::LinearizedState, body::RigidBody, point::Point3D, obstacle::Obstacle, model::Model)
    ...
    normal_velocity = linearized(xnext) do x
        dot(contact_normal(obstacle), point_velocity(twist_wrt_world(x, body), transform_to_root(linearization_state(xnext), point.frame) * point))
    end
    ...
    D_transpose_times_v = map(D) do d
        linearized(xnext) do x
            dot(d, point_velocity(twist_wrt_world(x, body), transform_to_root(linearization_state(xnext), point.frame) * point))
        end
    end
    # D_transpose_times_v = [
    ...
    add_contact_constraints(model, point, obstacle, β, λ, c_n, D_transpose_times_v, separation_from_obstacle, normal_velocity, Δr, Δr_true)
end
